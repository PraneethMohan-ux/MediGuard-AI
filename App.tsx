...
const App: React.FC = () => {
  ...
  useEffect(() => {
     if (profile) localStorage.setItem('mediguard_profile', JSON.stringify(profile));
     else localStorage.removeItem('mediguard_profile');
  }, [profile]);
    ...
    <div className={`flex flex-col h-screen w-full overflow-hidden relative bg-surface dark:bg-black transition-colors duration-500`}>
      ...
      <header className="p-4 flex items-center justify-between z-10 bg-surface/80 dark:bg-black/80 backdrop-blur-md sticky top-0 border-b border-gray-100 dark:border-neutral-800">
        <div className="flex items-center gap-4">
            ...
            <span className="font-bold text-lg text-primary dark:text-teal-400 hidden md:block">MediGuard AI</span>
        </div>
      ...
      {currentScreen !== 'chat' && (
        <button 
            ...
            className="fixed bottom-6 right-6 z-30 bg-primary dark:bg-teal-600 text-white p-4 rounded-full shadow-lg shadow-primary/30 hover:scale-110 transition-transform active:scale-95"
        >
