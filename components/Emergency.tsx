...
import { UserProfile, UI_TRANSLATIONS } from '../types';
...
const Emergency: React.FC<EmergencyProps> = ({ profile, language }) => {
  ...
  return (
    <div className="p-6 h-full flex flex-col items-center justify-center space-y-6 animate-pulse-slow">
      ...
      <div className="text-center space-y-1">
        <h2 className="text-3xl font-bold text-gray-900">{t.emergency}</h2>
        <p className="text-gray-500 text-sm">{t.immediateAssist}</p>
      </div>
      ...
      {profile && (profile.contacts[0].name || profile.contacts[1].name) && (
          <div className="w-full max-w-xs space-y-3">
              <p className="text-xs text-gray-400 font-bold uppercase tracking-wider text-center">{t.savedContacts}</p>
